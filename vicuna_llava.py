...
class vicuna_llava(LlamaForCausalLM):
    def __init__(self, config, llmURL="lmsys/vicuna-7b-v1.5", clipvisionmodelURL="openai/clip-vit-large-patch14"):
        super().__init__(config)

        # grab pretrained vicuna-7B-v1.5 for main LM functionality
        self.model = AutoModelForCausalLM.from_pretrained(llmURL)

        # vision tower
        self.im_processor = CLIPImageProcessor.from_pretrained(clipvisionmodelURL)
        self.clipvisionmodel = CLIPVisionModel.from_pretrained(clipvisionmodelURL)

        # tokenizer
        self.tokenizer = AutoTokenizer.from_pretrained(llmURL)
        self.special_tokens_dict = {"additional_special_tokens": ["<image>", "###"]}
        ...
        self.im_embedding = nn.Linear(1024, 4096)
    ...
    def forward(self, image:Tensor, prompt:str, batch_size=1):
        with torch.no_grad():
            ...
            clip_encoded_im = self.vision_tower(image)
        ...
        projected_im = self.im_embedding(clip_encoded_im)
    ...
    def generate(self, image:Tensor, prompt:str, max_new_tokens=50, batch_size=1):
        ...
        while num_tokens < max_new_tokens:
            outs = self.forward(image,prompt+pred_tokens,batch_size=1)

            pred_token_id = torch.argmax(outs[:,-1,:])
